...
parser.add_argument('--topk', type=int, default=20,
                    help='Top-K focused KL loss: only align draft distribution on target top-K tokens. '
                         'Set to 0 to use the original full-vocabulary KL loss.')
...
for epoch in range(args.start, args.start + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        if topk > 0:
            _, topk_indices = target_p.topk(topk, dim=-1)
            target_p_focused = torch.zeros_like(target_p)
            target_p_focused.scatter_(-1, topk_indices, target_p.gather(-1, topk_indices))
            target_p_focused = target_p_focused / (target_p_focused.sum(dim=-1, keepdim=True) + 1e-8)
            plogp = target_p_focused * out_logp
        else:
            # Original full-vocabulary KL loss
